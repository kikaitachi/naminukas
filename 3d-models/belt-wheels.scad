...
idler_height_p = 1; // [0.5:Tiny (0.5mm), 1:Small (1mm), 1.5:Normal (1.5mm), 2:Big (2mm), 3:Bigger (3mm), 4:Hudge (5mm)]
...
difference() {
    translate([0, 0, 3]) {
        cylinder(d = motor_shaft_diameter + 1, h = 2, $fn = 100);
    }
    translate([0, 0, 2.999]) {
        cylinder(d1 = motor_shaft_diameter + 0.5, d2 = motor_shaft_diameter, h = 1.01, $fn = 100);
    }
    translate([0, 0, 3.999]) {
        cylinder(d1 = motor_shaft_diameter, d2 = motor_shaft_diameter + 0.5, h = 1.01, $fn = 100);
    }
}